...
if(!function_exists('magazinevibe_edge_get_title')) {
    /**
     * Loads title area HTML
     */
    function magazinevibe_edge_get_title() {
        $id = magazinevibe_edge_get_page_id();

        extract(magazinevibe_edge_title_area_height());
        extract(magazinevibe_edge_title_area_background());
        ...
        if(get_post_meta($id, "edgtf_show_title_area_meta", true) !== ""){
            ...
            $show_title_area = magazinevibe_edge_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        $parameters = apply_filters('magazinevibe_edge_title_area_height_params', $parameters);

        magazinevibe_edge_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('magazinevibe_edge_get_title_text')) {
    /**
     * Function that returns current page title text. Defines magazinevibe_edge_title_text filter
     ...
     * @see magazinevibe_edge_is_woocommerce_installed()
     */
    function magazinevibe_edge_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false) . esc_html__(' Tag', 'magazinevibe');
        }

        //is current page date archive?
        elseif (is_date()) {
            //get current date archive format
            $title = get_the_time('M d, Y');
        }

        //is current page author archive?
        elseif (is_author()) {
            //get current author name
            $title = esc_html__('Author:', 'magazinevibe') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search', 'magazinevibe');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
            if(magazinevibe_edge_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = magazinevibe_edge_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'magazinevibe');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(magazinevibe_edge_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(magazinevibe_edge_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','magazinevibe');
        }
        ...
        $title = apply_filters('magazinevibe_edge_title_text', $title);

        return $title;
    }
}

if(!function_exists('magazinevibe_edge_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see magazinevibe_edge_get_title_text()
     */
    function magazinevibe_edge_title_text() {
        echo magazinevibe_edge_get_title_text();
    }
}

if(!function_exists('magazinevibe_edge_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see magazinevibe_edge_is_product_category()
     * @see get_search_query()
     */
    function magazinevibe_edge_custom_breadcrumbs() {
        global $post, $wp_query;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = magazinevibe_edge_get_page_id();
        $bread_style = "";

        if(get_post_meta($pageid, "edgtf_title_breadcrumb_color_meta", true) != ""){
            $bread_style="color:". get_post_meta($pageid, "edgtf_title_breadcrumb_color_meta", true);
        }

        $showOnHome = 1; // 1 - show breadcrumbs on the homepage, 0 - don't show
        $delimiter = '<span class="edgtf-delimiter" '.magazinevibe_edge_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'magazinevibe'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="edgtf-current" '.magazinevibe_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner"><a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs" itemprop="breadcrumb"><div class="edgtf-breadcrumbs-inner"><a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;
            ...
            if ( is_category()) {
                ...
                $output .= $before . esc_html__('Search', 'magazinevibe') . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.magazinevibe_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts Tagged ', 'magazinevibe') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles Posted by ', 'magazinevibe') . $authordata->display_name . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404', 'magazinevibe') . $after;
            } elseif(magazinevibe_edge_is_woocommerce_installed() && is_shop()){
                global $woocommerce;
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'magazinevibe') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('magazinevibe_edge_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function magazinevibe_edge_get_title_area_height() {
        ...
        } elseif(magazinevibe_edge_options()->getOptionValue('title_area_height') !== '') {
            $title_height = magazinevibe_edge_options()->getOptionValue('title_area_height');
        }

        return apply_filters('magazinevibe_edge_title_area_height', $title_height);
    }
}

if(!function_exists('magazinevibe_edge_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function magazinevibe_edge_get_title_content_padding() {
        return apply_filters('magazinevibe_edge_title_content_padding', 0);
    }
}


if(!function_exists('magazinevibe_edge_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function magazinevibe_edge_title_area_height() {
        $id = magazinevibe_edge_get_page_id();
        $title_height_and_padding = array();
        $title_height          = magazinevibe_edge_get_title_area_height();
        $header_height_padding = magazinevibe_edge_get_title_content_padding();
        ...
        if(get_post_meta($id, "edgtf_title_area_vertial_alignment_meta", true) !== '') {
            ...
            $title_vertical_alignment = magazinevibe_edge_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('magazinevibe_edge_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function magazinevibe_edge_title_area_background() {
        $id = magazinevibe_edge_get_page_id();
        $show_title_img = true;
        ...
        if(get_post_meta($id, "edgtf_title_area_background_color_meta", true) != ""){
            ...
            $background_color = magazinevibe_edge_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(get_post_meta($id, "edgtf_title_area_background_image_meta", true) != ""){
            ...
            $background_image = magazinevibe_edge_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = magazinevibe_edge_get_image_dimensions($background_image);
