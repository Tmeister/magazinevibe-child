<?php
namespace MagazineVibe\Modules\Shortcodes\Icon;

use MagazineVibe\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package MagazineVibe\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Edge Icon', 'magazinevibe'),
            'base'                      => $this->base,
            'category'                  => 'by EDGE',
            'icon'                      => 'icon-wpb-icons extended-custom-icon',
            'allowed_container_element' => 'vc_row',
            'params'                    => array_merge(
                \MagazineVibeIconCollections::get_instance()->getVCParamsArray(),
                array(
                    array(
                        'type'        => 'dropdown',
                        'heading'     => 'Size',
                    ...
                    array(
                        ...
                        'description' => esc_html__('Please insert border radius(Rounded corners) in px. For example: 4 ', 'magazinevibe'),
    ...
    public function render($atts, $content = null) {
        ...
        $default_atts = array_merge($default_atts, magazinevibe_edge_icon_collections()->getShortcodeParams());
        $params       = shortcode_atts($default_atts, $atts);

        $iconPackName = magazinevibe_edge_icon_collections()->getIconCollectionParamNameByKey($params['icon_pack']);
        ...
        $html = magazinevibe_edge_get_shortcode_module_template_part('templates/icon', 'icon', '', $params);
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.magazinevibe_edge_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.magazinevibe_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.magazinevibe_edge_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.magazinevibe_edge_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.edgtf_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.magazinevibe_edge_filter_px($params['border_radius']).'px';
                }
            }
        }
